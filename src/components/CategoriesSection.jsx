          ...
          {categories.map((category, index) => {
            ...
            return (
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className="category-icon group flex flex-col items-center p-2 sm:p-2.5 bg-white rounded-lg border border-gray-200 cursor-pointer shadow-sm hover:shadow-md hover:border-blue-500 transition-all duration-200"
                initial={{ opacity: 0, y: 15 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.04 }}
              >
                <Link to={`/category/${category.id}`} className="flex flex-col items-center w-full">
                  <div className="bg-slate-100 p-2 sm:p-2.5 rounded-full mb-1.5 transition-colors duration-200 group-hover:bg-blue-500">
